...
\begin{document}
    ...
    Единственный случай, когда $\Var(Z)=0$, это $Z\equiv c$. Тогда $X+Z = X+c$ и $Y+Z = Y+c$, и они независимы как сдвиги независимых $X$ и $Y$.

    \medskip
    \noindent
    \textbf{Вывод:} вообще говоря, $X+Z$ и $Y+Z$ не являются независимыми. Они независимы только в случае, когда $Z$ почти наверное константа.
    ...
    \begin{tikzpicture}[scale=5]
        ...
        % Треугольник поддержки: 0 ≤ y' ≤ x' ≤ 1
        \fill[gray!20] (0,0) -- (1,0) -- (1,1) -- cycle;

        % Линия y'=x'
        \draw[thick] (0,0) -- (1,1) node[anchor=south west] {$y'=x'$};
        ...
        % Текст
        \node at (0.6,0.15) {$f_{X,Y}(x',y')>0$};
        \node at (0.7,0.6) {$y' \le x'$};
    ...
    \begin{tikzpicture}[scale=5]
        ...
        \fill[gray!15] (0,0) -- (1,0) -- (1,1) -- cycle;
        \draw[thick] (0,0) -- (1,1) node[anchor=south west] {$y'=x'$};

        % Прямоугольник X ≤ x, Y ≤ y
        \draw[fill=yellow!30, opacity=0.6]
        (0,0) rectangle (\x,\y);

        % Часть 1: треугольник 0 ≤ x' ≤ y, 0 ≤ y' ≤ x'
        \fill[blue!40, opacity=0.6]
        (0,0) -- (\y,0) -- (\y,\y) -- cycle;

        % Часть 2: "полоса" y ≤ x' ≤ x, 0 ≤ y' ≤ y
        \fill[blue!70, opacity=0.4]
        (\y,0) -- (\x,0) -- (\x,\y) -- (\y,\y) -- cycle;

        % Пунктирные линии к (x,y)
        \draw[dashed, red] (\x,0) -- (\x,\y);
        \draw[dashed, red] (0,\y) -- (\x,\y);

        % Подписи контрольных точек
        \filldraw[red] (\x,\y) circle (0.006);
        \node[red, anchor=south west] at (\x,\y) {$\,(x,y)$};

        \node[below left] at (0,0) {$(0,0)$};
        \node[below]      at (\x,0) {$(x,0)$};
        \node[below]      at (\y,0) {$(y,0)$};
        \node[left]       at (\y,\y) {$(y,y)$};
        \node[left]       at (\x,\y) {$(x,y)$};

        % Текст пояснений
        \node at (0.15,0.07) {\small часть 1};
        \node at (0.15,0.13) {\small $x' \in [0,y]$};
        \node at (0.15,0.19) {\small $y' \in [0,x']$};

        \node at (0.55,0.12) {\small часть 2};
        \node at (0.55,0.18) {\small $x' \in [y,x]$};
        \node at (0.55,0.24) {\small $y' \in [0,y]$};

        \node at (0.75,0.15) {\small поддержка};
        \node at (0.8,0.6)  {\small $y'\le x'$};
